...
active = 'PYTHON_PRETTY_ERRORS' not in os.environ or os.environ[
    'PYTHON_PRETTY_ERRORS'] != '0'
interactive_tty_only = 'PYTHON_PRETTY_ERRORS_ISATTY_ONLY' in os.environ and \
                       os.environ['PYTHON_PRETTY_ERRORS_ISATTY_ONLY'] != '0'
...
class PrettyErrorsConfig(object):
...
class ExceptionWriter(object):
    ...
    def write_header(self):
        ...
        if not self.config.separator_character:
            return
        line_length = self.get_line_length()
        if self.config.display_timestamp:
            timestamp = str(self.config.timestamp_function())
            separator = (line_length - len(
                timestamp)) * self.config.separator_character + int(timestamp)
        else:
    ...
    def write_location(self, path, line, function):
        ...
        if self.config.filename_display == FILENAME_FULL:
            ...
            self.output_text([self.config.line_number_color, line_number,
                              self.config.function_color, function])
        else:
            if self.config.filename_display == FILENAME_EXTENDED:
                line_length = self.get_line_length()
                filename = path[-(line_length - len(line_number) - len(
                    function) - 4):]
                if filename != path:
                    filename = '...' + filename
            else:
    ...
    def write_link(self, filepath, line):
        ...
        self.output_text(
            [self.config.link_color, '"%s", line %s' % (filepath, line)])
    ...
    def write_code(self, filepath, line, module_globals, is_final,
        ...
        if filepath == '<stdin>':
            ...
            for i in range(start, end + 1):
                lines.append(
                    linecache.getline(filepath, i, module_globals).rstrip())
        ...
        for i, line in enumerate(lines):
            ...
            if self.config.truncate_code and len(
                    line) + color_length > line_length:
            ...
            if i == target_line and point_at is not None:
                ...
                self.output_text([
                    ...
                    self.config.syntax_error_color, line[start_char:end_char],
                    RESET_COLOR,
                    color, line[end_char:]
                ])
                if self.config.display_arrow:
                    self.output_text([
                        self.config.arrow_tail_color,
                        self.config.arrow_tail_character * point_at,
                        self.config.arrow_head_color,
                        self.config.arrow_head_character
                    ])
            else:
    ...
    def write_exception(self, exception_type, exception_value):
        ...
        if exception_value and len(exception_value.args) > 0:
            ...
            output = [self.config.exception_color,
                      self.exception_name(exception_type)]
...
def excepthook(exception_type, exception_value, traceback):
    ...
    if (
            not writer.config.top_first and
            exception_value.__context__ and
            (not exception_value.__suppress_context__ or config.show_suppressed)
    ):
        excepthook(
            type(exception_value.__context__),
            exception_value.__context__,
    ...
    def check_for_pathed_config(path_):
        writer.config = writer.default_config
        for config_path in config_paths:
            if path_.startswith(config_path):
    ...
    if traceback:
        ...
        while tb.tb_next is not None:
            tb = tb.tb_next
        check_for_pathed_config(
            os.path.normpath(tb.tb_frame.f_code.co_filename).lower())
    ...
    if syntax_error_info:
        ...
        if syntax_error_info[0] == '<stdin>':
            writer.write_code(syntax_error_info[0], syntax_error_info[3], [],
                              True, syntax_error_info[2])
        else:
            writer.write_code(syntax_error_info[0], syntax_error_info[1], [],
                              True, syntax_error_info[2])
    else:
        tracebacks = []
        while traceback is not None:
            path = os.path.normpath(
                traceback.tb_frame.f_code.co_filename).lower()
            if traceback.tb_next is None or (
                    writer.config.always_display_bottom and tracebacks == []):
        ...
        if writer.config.top_first:
            ...
            if writer.config.stack_depth > 0:
                if writer.config.always_display_bottom and len(tracebacks) > 1:
                    tracebacks = tracebacks[
                                 :writer.config.stack_depth] + tracebacks[-1:]
                else:
            ...
            if writer.config.stack_depth > 0:
                if writer.config.always_display_bottom and len(tracebacks) > 1:
                    tracebacks = tracebacks[:1] + tracebacks[
                                                  -writer.config.stack_depth:]
                else:
                    tracebacks = tracebacks[-writer.config.stack_depth:]
        ...
        for count, traceback in enumerate(tracebacks):
            ...
            code = frame.f_code
            writer.write_location(code.co_filename, traceback.tb_lineno,
                                  code.co_name)
            code_string = writer.write_code(code.co_filename,
                                            traceback.tb_lineno,
                                            frame.f_globals, count == final)

            if (writer.config.display_locals and count == final) or (
                    writer.config.display_trace_locals and count != final):
                local_variables = [(code_string.find(x), x) for x in
                                   frame.f_locals]
                ...
                if local_variables:
                    ...
                    for local in local_variables:
                        ...
                        output = [
                            writer.config.local_name_color,
                            local,
                            spacer,
                            writer.config.local_value_color
                        ]
                        local_len = len(local)
                        spacer_len = len(spacer)
                        value_len = len(value)
                        if all([
                            writer.config.truncate_locals,
                            sum([
                                local_len, spacer_len, value_len
                            ]) > line_length
                        ]):
                            ...
                            output += [
                                value, len_spacer,
                                writer.config.local_len_color, length
                            ]
                        else:
                            output += [value]
    ...
    if (
        ...
        if writer.config.inner_exception_message is not None:
            if writer.config.inner_exception_separator:
                writer.write_header()
            output_stderr.write(writer.config.inner_exception_message)
        excepthook(type(exception_value.__context__),
                   exception_value.__context__,
                   exception_value.__context__.__traceback__)
...
class StdErr(object):
    """
    Replaces `sys.stderr` in order to scrape it, when capturing stack trace is
    unavailable.
    """
    ...
    def __getattr__(self, name_):
        return getattr(output_stderr, name_)

    def __enter__(self, *args, **kwargs):
        return output_stderr.__enter__()

    @staticmethod
    def __is_header(text):
        """Is text a traceback header?"""
        return text.startswith('Traceback (most recent call last):')

    @staticmethod
    def __is_outer_exception(text):
    ...
    @staticmethod
    def __get_location(text):
        """
        Extract location of exception.
        If it returns None then text was not a location identifier.
        """
        location = location_expression.match(text)
        if location:
            return (
                location.group(1), int(location.group(2)), location.group(3))
        else:
            return None
    ...
    def __write_exception(self):
        ...
        if syntax_error_info:
            check_for_pathed_config(
                os.path.normpath(syntax_error_info[0]).lower())
            writer.write_location(syntax_error_info[0], syntax_error_info[1],
                                  '')
            writer.write_code(syntax_error_info[0], syntax_error_info[1], [],
                              True, syntax_error_info[2])
            ...
            if writer.config.top_first:
                tracebacks.reverse()
                if writer.config.stack_depth > 0:
                    if writer.config.always_display_bottom and len(
                            tracebacks) > 1:
                        tracebacks = tracebacks[
                                     :writer.config.stack_depth] + tracebacks[
                                                                   -1:]
                    else:
                        tracebacks = tracebacks[:writer.config.stack_depth]
                final = 0
                ...
                if writer.config.stack_depth > 0:
                    if all([
                        writer.config.always_display_bottom,
                        len(tracebacks) > 1
                    ]):
                        tracebacks = (
                                tracebacks[:1] +
                                tracebacks[-writer.config.stack_depth:]
                        )
                    else:
                        tracebacks = tracebacks[-writer.config.stack_depth:]
                final = len(tracebacks) - 1
                ...
                if self.__awaiting_end > 0:
                    ...
                    if not line.strip():
                        ...
                        if location:
                            ...
                            if ': ' in line:
                                self.__exception, self.__exception_args = line.strip().split(
                                    ': ', 1)
                                self.__in_exception = False
                                self.__write_exception()
                            else:
                    ...
                    if config.inner_exception_message is not None:
...
def replace_stderr(force=False):
    """
    Replace `sys.stderr`, for cases where standard use with `activate()` does
    not work.
    """
